...
class Reuters(Dataset):
    def __init__(self, 
                 root_dir:str="G:\Data",
                 **kwargs):
        data = np.load(os.path.join(root_dir, "REUTERS10K", 'reutersidf10k.npy'), allow_pickle=True).item()
        self.name="reuters10k"
        self.features = data['data']
        self.labels   = data['label']
        self.is_image = False
        self.feature_dim = 2000
        self.feature_shape = [2000]
    ...
    def split(self, perscentages=[]):
        ...
        datasets=random_split(self, splits_size)
        for data in datasets:
            for key, val in data.dataset.__dict__.items():
...
class tvsDataset(Dataset):
    def __init__(self, 
        ...
        if name in ['STL10', 'CelebA', 'SVHN']: 
            ...
            data_kwargs=dict(root=root_dir, split="train" if train else "test", transform=img_transform)
            ...
            # img_transform = transforms.Compose([transforms.Lambda(self._transformation)])
            data_kwargs=dict(root=root_dir, train=True if train else False, transform=img_transform)
            self.ds = getattr(datasets, name)(**data_kwargs)

        self.feature_shape=self.ds[0][0].shape
        self.is_imgage = True if (self.feature_shape[-1]==self.feature_shape[-2] and self.feature_shape[-3] in [1,3]) else False
        self.feature_dim=self.ds[0][0].shape[1] if self.is_imgage else self.ds[0][0].shape[-1]
...
class procDataset(Dataset):
    def __init__(self, 
        ...
        self.features = torch.from_numpy(self.data['features'])
        if is_normalize:
            self.features = normalize(self.features, dim=0)
        self.labels = torch.from_numpy(self.data['labels']).squeeze()
        self.dataset= TensorDataset(self.features.to(torch.float32),
                                    self.labels.to(torch.long))  
        self.clusters=self.labels.unique().tolist() if self.data.get('clusters') is None else self.data['clusters']
        self.n_cluster=len(self.clusters)
        self.n_sample = len(self.dataset)
...
def LoadDataset(params):   
    if params.use_processed_data: 
        if params["name"].lower()=="reuters10k":
            traindata, testdata = Reuters(**params).split()
        else:
            ...
            if not os.path.exists(is_done):    
                ...
                to_process = dict(train=tvsDataset(train=True,**params), 
                                test=tvsDataset(train=False,**params))
