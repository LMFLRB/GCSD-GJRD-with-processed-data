...
from models.dae import DenoisingAutoencoder as DAE
...
class Experiment():
    ...
    def set_version(self, log_dir:str=None, version=None):
        ...
        self.logdir_cluster = f"{self.log_dir}/version_{version}" 
        self.log_text_path = f"{self.log_dir}/{self.params['log_text_path']}.txt"
    ...
    def evaluate(self, model, dataset, save_results:bool=True, visualize:bool=False,
        ...
        if ckpt_mode in ["acc", "nmi", "ari"]:            
            ckpt_path=f"{ckpt_path}/{ckpt_mode}_best.ckpt" if len(ckpt_path)>0 else f"{self.logdir_cluster}/{ckpt_mode}_best.ckpt"
            if os.path.exists(ckpt_path):
                model.load_state_dict(torch.load(ckpt_path))
            else:
                model.load_state_dict(getattr(self, f"{ckpt_mode}_best_ckpt"))
            mode="best"
